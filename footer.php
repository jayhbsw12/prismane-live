...
var numberParticlesStart = jsonMap.length,
    ...
    x = 0,
    y = 0,
    particleFillColor = '#121212',
    particleStrokeColor = '#121212', 
    particleLinkColor = '#fff00',
    showZone = false;

/* ---- INIT ---- */
var mouse = { x: null, y: null, radius: 50 };
var particles = [];
...
Particle.prototype.renderLinks = function(particles) {
    ...
    _.each(particles, function(p) {
        ...
        if (distance < distanceLinkMax && that.nbrOfLinks < linkMax && p.nbrOfLinks < linkMax) {
            ...
            var midX = (p.x + that.x) / 2;
            var midY = (p.y + that.y) / 2;

            // Distance from mouse to the line midpoint
            var mouseDistance = Math.sqrt(Math.pow(mouse.x - midX, 2) + Math.pow(mouse.y - midY, 2));
            ...
            if (mouseDistance < mouse.radius) {
                context.strokeStyle = "#c9f31d";  // yellow on hover
                context.lineWidth = 0.6;
            } else {
                context.strokeStyle = "#999";  // default gray
